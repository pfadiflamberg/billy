...
For user authentication and to access hitobito the app needs to be registered as an oauth application on hitobito. The application needs all available scopes and a callback URL (REDIRECT_URL_LOGIN) of the following form `API_SERVER/oauth/billy/authorized` (e.g. `http://localhost:6000/oauth/billy/authorized `).
